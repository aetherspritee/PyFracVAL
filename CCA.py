    ...
    if gamma_real and ext_case == 1:
        for i in range(curr_list.shape[0]-1):
            ...
            for j in range(curr_list.shape[1]-1):
                d_j_min = np.sqrt(np.power(X2[j]-X_cm2,2) + np.power(Y2[j]-Y_cm2,2) + np.power(Z2[j]-Z_cm2,2)) - R2[j]
                d_j_max = np.sqrt(np.power(X2[j]-X_cm2,2) + np.power(Y2[j]-Y_cm2,2) + np.power(Z2[j]-Z_cm2,2)) + R2[j]
        ...
        for i in range(curr_list.shape[0]-1):
            ...
            for j in range(curr_list.shape[1]-1):
                d_j_max = np.sqrt(np.power(X2[j]-X_cm2,2) + np.power(Y2[j]-Y_cm2,2) + np.power(Z2[j]-Z_cm2,2)) + R2[j]
                if d_i_max + d_j_max > gamma_pc and np.abs(d_j_max-d_i_max) < gamma_pc:
                    curr_list[i,j] = 1
                        

    return curr_list

def CCA(X: np.ndarray,Y: np.ndarray,Z: np.ndarray,R: np.ndarray, N: int, ID_mon: np.ndarray, k: int,other: int, Df: float, kf: float,ext_case, tolerance: float):
    CCA_ok = True
...
def CCA_random_pick(curr_list: np.ndarray, prev_cand1: int, prev_cand2=None):
    ...
    if prev_cand2 is None:
        ...
        selected = np.random.rand() * curr_list2.size
        sel = 0
        jj = 0
...
def CCA_sticking_process(gamma_real: bool, gamma_pc: float, COR1,COR2,CM1,CM2, prev_cand1: int, prev_cand2: int, ext_case: int,n1: int, n2: int):
    if gamma_real:
        ...
        vect_mag = np.sqrt(np.power(vect_x,2) + np.power(vect_y,2) + np.power(vect_z,2))
...
def random_point_SC(case: int,sphere1: np.ndarray, sphere2: np.ndarray):
    ...
    if case == 1:
        ...
        phi_crit_min = 0
        sph1_r = sph1_r_min
    theta_r = 2*np.pi*np.random.rand()
    phi_r = phi_crit_min + (phi_crit_max-phi_crit_min)*np.random.rand()
...
def spherical_cap_angle(sphere1: np.ndarray, sphere2: np.ndarray):
    ...
    D = np.power(sphere1[0],2)-np.power(sphere2[0],2) + np.power(sphere1[1],2)-np.power(sphere2[1],2) + np.power(sphere1[2],2)-np.power(sphere2[2],2) - np.power(sphere1[3],2)+np.power(sphere2[3],2)

    t = (sphere1[0]*A + sphere1[1]*B  + sphere1[2]*C + D)/(A*(sphere1[0]-sphere2[0]) + B*(sphere1[1]-sphere2[1]) + C*(sphere1[2]-sphere2[2]))
    ...
    lc_cm1 = np.sqrt(np.power(t*(sphere2[0]-sphere1[0]),2) + np.power(t*(sphere2[1]-sphere1[1]),2) + np.power(t*(sphere2[2]-sphere1[2]),2))
...
def CCA_2_sphere_intersec(sphere1: np.ndarray, sphere2: np.ndarray):
    ...
    k_vec = np.array([A,B,C])/np.sqrt(np.power(A,2) + np.power(B,2) + np.power(C,2))
...
def CCA_sticking_process_v2(CM2: np.ndarray, vec0: np.ndarray, X2_new,Y2_new,Z2_new, i_vec, j_vec, prev_cand):
    theta_a = 2*np.pi * np.random.rand()
    ...
    z = vec0[2] + vec0[3] * np.cos(theta_a) * i_vec[2] + vec0[3] * np.sin(theta_a) * j_vec[2]
