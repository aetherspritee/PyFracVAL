...
class OrchestratorAlgorithmConfig(BaseModel):
    ...
    # NOTE: there is deliberately no "use masses vs counts" flag here.
    # Mass weighting is controlled entirely by the optional per-particle
    # `densities` argument (None -> uniform, so mass is proportional to
    # r^3). Which form each stage solves is a property of that stage's
    # geometry, not a user preference - see fractal.gamma_calculation and
    # docs/source/backtracking_pairing.md.
