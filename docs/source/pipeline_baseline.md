...
| CCA drop-rescue (`cca_drop_rescue_enabled`) | opt-in | implemented and benchmarked - default (conservative) budget has zero effect at N=128 hard regime; a relaxed budget triples single-shot success (2.5%→7.5%) with no obvious fractal-accuracy penalty on the small sample rescued | [drop_rescue.md](drop_rescue.md) |
