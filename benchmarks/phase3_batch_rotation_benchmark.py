#!/usr/bin/env python3
"""
Phase 3 Batch Rotation Benchmark

Compares Phase 1 (sequential rotations) vs Phase 3 (batch parallel rotations).
Measures speedup from batch evaluation with Numba prange.

Test configurations:
- N=128, 256, 512 with optimal parameters
- 3 trials each for statistical significance
"""
    ...
    for trial in range(trials):
        ...
        try:
            success, coords, radii = run_simulation(
                1, config, output_base_dir="/tmp/phase3_bench", seed=seed
            )
...
def main():
    ...
    print("=" * 80)
    print("Phase 3: Batch Rotation Benchmark")
    print("=" * 80)
    ...
    print(f"Optimization: Batch rotation with Numba prange")
    print()

    # Test configurations (same as phase comparison)
    configs = [
        {
            "name": "N=128, Optimal",
            "N": 128,
    ...
    # Save results
    output_file = Path(f"benchmark_results/phase3_batch_rotation_{commit}.json")
        ...
        json.dump(
            {
                ...
                "optimization": "Batch rotation with Numba prange",
                "results": results,
            },
            f,
            indent=2,
    ...
    if phase1_file.exists():
        ...
        print("\n" + "=" * 80)
        print("SPEEDUP vs PHASE 1")
        print("=" * 80)

        for name, result in results.items():
            if result and name in phase1_data["results"]:
                phase1_time = phase1_data["results"][name]["avg_runtime"]
                phase3_time = result["avg_runtime"]
                speedup = phase1_time / phase3_time
                print(f"{name:25s} {speedup:.2f}x faster")
