#!/usr/bin/env python3
"""
Phase 4A Bounding Spheres Benchmark

Measures performance improvement from bounding sphere fast rejection in CCA.
Tests the optimization where we skip expensive particle-level overlap checks
when bounding spheres don't intersect.

Compares against Phase 1 baseline (sequential without bounding spheres).
"""
    ...
    for trial in range(trials):
        ...
        try:
            success, coords, radii = run_simulation(
                1, config, output_base_dir="/tmp/phase4a_bench", seed=seed
            )
...
def main():
    """Run Phase 4A benchmark."""
    print("=" * 80)
    print("Phase 4A: Bounding Spheres Benchmark")
    print("CCA Fast Rejection Optimization")
    print("=" * 80)
    ...
    # Save results
    output_file = Path(f"benchmark_results/phase4a_bounding_spheres_{commit}.json")
        ...
        json.dump(
            {
                ...
                "optimization": "CCA bounding sphere fast rejection",
                "description": "Single sphere check before n1×n2 particle checks",
                "results": results,
            },
            f,
            indent=2,
    ...
    if phase1_file.exists():
        ...
        print("\n" + "=" * 80)
        print("SPEEDUP vs PHASE 1 (Sequential baseline)")
        print("=" * 80)

        for name, result in results.items():
            if result and name in phase1_data["results"]:
                phase1_time = phase1_data["results"][name]["avg_runtime"]
                phase4a_time = result["avg_runtime"]
                speedup = phase1_time / phase4a_time
                print(f"{name:25s} {speedup:.2f}x")

        print("\nNote: Speedup depends on how often bounding spheres reject early")
        print(
            "      Expected: 1.5-10x for clusters that don't overlap (geometry dependent)"
        )
