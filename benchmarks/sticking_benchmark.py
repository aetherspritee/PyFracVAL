...
class StickingBenchmark:
        ...
        return [
            {
                ...
                "kf": 1.0,
                "rp_gstd": 1.5,
                "description": "Original paper example",
            },
            {
                "N": 128,
                "Df": 2.0,
            ...
            },
            {
                ...
                "kf": 1.2,
                "rp_gstd": 1.3,
                "description": "Moderate polydisperse",
            },
        ]
        ...
        return [
            {
                ...
                "kf": 1.0,
                "rp_gstd": 1.8,
                "description": "High polydispersity",
            },
            {
                "N": 128,
                "Df": 1.8,
                "kf": 1.0,
                "rp_gstd": 2.0,
                "description": "Very high polydispersity",
            },
            {
                "N": 128,
                "Df": 1.8,
            ...
            },
            {
                ...
                "kf": 1.0,
                "rp_gstd": 1.8,
                "description": "High Df + polydisperse",
            },
        ]
    ...
    def run_suite(
        ...
        for test_params in test_cases:
            ...
            print(
                f"Parameters: N={test_params['N']}, Df={test_params['Df']}, "
            ...
            print(
                f"  Success rate: {successes}/{n_trials} ({100 * successes / n_trials:.1f}%)"
            )
            print(f"  Avg runtime: {avg_runtime:.2f}s")

            if save_individual:
                self._save_case_results(
                    category, test_params["description"], case_results
                )
    ...
    def _save_case_results(
        self, category: str, description: str, results: List[BenchmarkResult]
    ):
        """Save results for a single test case."""
        output_file = (
            self.output_dir / f"{category}_{description.replace(' ', '_')}.json"
        )
    ...
    def _generate_final_report(self, all_results: Dict[str, List[BenchmarkResult]]):
        ...
        with open(report_file, "w") as f:
            ...
            f.write(
                "| Category | Total Trials | Successes | Success Rate | Avg Runtime |\n"
            )
            f.write(
                "|----------|--------------|-----------|--------------|-------------|\n"
            )
            ...
            for category, results in all_results.items():
                ...
                f.write(
                    f"| {category} | {total} | {successes} | {rate:.1f}% | {avg_time:.2f}s |\n"
                )
            ...
            for category, results in all_results.items():
                ...
                for desc, case_results in test_cases.items():
                    ...
                    f.write(f"**{desc}**\n")
                    f.write(
                        f"- Parameters: N={r0.N}, Df={r0.Df}, kf={r0.kf}, rp_gstd={r0.rp_gstd}\n"
                    )
                    f.write(
                        f"- Success: {successes}/{total} ({100 * successes / total:.1f}%)\n"
                    )
                    f.write(
                        f"- Avg Runtime: {np.mean([r.runtime_seconds for r in case_results]):.2f}s\n"
                    )
