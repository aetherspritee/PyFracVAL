...
- [x] ~~Further parallelization exploration~~ — evaluated JAX/GPU as an alternative to the Numba JIT hot path (overlap checks, cluster rotation): numba wins at every problem size tested, by 1-4 orders of magnitude, because its early-exit/bounding-sphere pre-check algorithmic tricks have no SIMD/GPU equivalent, and fixed per-call dispatch/launch overhead dominates at PyFracVAL's realistic array sizes. See [`docs/source/gpu_acceleration.md`](docs/source/gpu_acceleration.md) for the data. Numba JIT remains the production backend.
